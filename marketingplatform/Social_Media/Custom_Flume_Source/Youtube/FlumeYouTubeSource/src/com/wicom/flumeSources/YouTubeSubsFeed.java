...
public class YouTubeSubsFeed extends AbstractSource implements EventDrivenSource,Configurable, PollableSource {
	  ...
	  private void getSubscriptions() {
		  ...
		  if(TimeUnit.MILLISECONDS.toSeconds(currentTime - lastPoll) > pollInterval){
			  try {
				    ...
				    bearerClientRequest = new OAuthBearerClientRequest("https://www.googleapis.com/youtube/v3/subscriptions?part=contentDetails%2Cid%2Csnippet%2CsubscriberSnippet&mine=true&maxResults=50" + "&access_token=" + accessToken)
				    .buildQueryMessage();
					...
					logger.debug(bearerClientRequest.getLocationUri());
						...
						Event e = EventBuilder.withBody(value.getBytes());
						e.getHeaders().put(selectorHeader, "Subs");
						getChannelProcessor().processEvent(e);
						JSONObject json = (JSONObject) JSONSerializer.toJSON(resourceResponse.getBody().toString());     
						
						while(json.containsKey("nextPageToken"))
						   {
							bearerClientRequest = new OAuthBearerClientRequest("https://www.googleapis.com/youtube/v3/subscriptions?part=contentDetails%2Cid%2Csnippet%2CsubscriberSnippet&mine=true&maxResults=50" + "&access_token=" + accessToken + "&pageToken=" + json.getString("nextPageToken"))
						    .buildQueryMessage();
							bearerClientRequest.addHeader("x-li-format", "json");
							logger.debug(bearerClientRequest.getLocationUri());
