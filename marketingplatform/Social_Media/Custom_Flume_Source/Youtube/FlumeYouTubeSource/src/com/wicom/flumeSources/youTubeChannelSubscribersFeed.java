...
public class youTubeChannelSubscribersFeed extends AbstractSource implements EventDrivenSource,Configurable, PollableSource {
	  ...
	  private void getSubscribers() {
		  ...
		  if(TimeUnit.MILLISECONDS.toSeconds(currentTime - lastPoll) > pollInterval){
			  try {
				    ...
				    bearerClientRequest = new OAuthBearerClientRequest("https://www.googleapis.com/youtube/v3/channels?part=id%2C+snippet%2C+contentDetails%2C+statistics%2C+topicDetails%2C+invideoPromotion&maxResults=50&mySubscribers=true&access_token=" + accessToken)
				    .buildQueryMessage();
					...
					logger.debug(bearerClientRequest.getLocationUri());
						...
						Event e = EventBuilder.withBody(value.getBytes());
						e.getHeaders().put(selectorHeader, "Subsribers");
						getChannelProcessor().processEvent(e);
						JSONObject json = (JSONObject) JSONSerializer.toJSON(resourceResponse.getBody().toString());     
						
						while(json.containsKey("nextPageToken"))
						   {
							bearerClientRequest = new OAuthBearerClientRequest("https://www.googleapis.com/youtube/v3/channels?part=id%2C+snippet%2C+contentDetails%2C+statistics%2C+topicDetails%2C+invideoPromotion&&maxResults=50mySubscribers=true&access_token=" + accessToken + "&pageToken=" + json.getString("nextPageToken"))
						    .buildQueryMessage();
							bearerClientRequest.addHeader("x-li-format", "json");
							logger.debug(bearerClientRequest.getLocationUri());
