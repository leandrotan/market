...
public class YouTubeVideoStatistics extends AbstractSource implements EventDrivenSource,Configurable, PollableSource{
	  ...
	  private String getVideoStatistics() {
		  ...
		  if(TimeUnit.MILLISECONDS.toSeconds(currentTime - lastPoll) > pollInterval){
			  try {
				    ...
				    for(int i = 0 ; i < JAVL.size() ; i++){
				    	VideoIds= VideoIds + "," + (JAVL.getJSONObject(i).getJSONObject("contentDetails").getString("videoId"));
				    }
					  bearerClientRequest = new OAuthBearerClientRequest("https://www.googleapis.com/youtube/v3/videos?part=id%2Csnippet%2CcontentDetails%2CfileDetails%2CliveStreamingDetails%2Cplayer%2CprocessingDetails%2CrecordingDetails%2Cstatistics%2Cstatus%2Csuggestions%2CtopicDetails&maxResults=50&id=" + VideoIds + "&access_token=" + accessToken)
					    .buildQueryMessage();
						bearerClientRequest.addHeader("x-li-format", "json");
						logger.debug(bearerClientRequest.getLocationUri());
							resourceResponse = oAuthClient.resource(bearerClientRequest, OAuth.HttpMethod.GET, OAuthResourceResponse.class);
