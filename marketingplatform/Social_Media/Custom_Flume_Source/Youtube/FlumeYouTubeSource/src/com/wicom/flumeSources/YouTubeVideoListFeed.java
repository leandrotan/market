...
public class YouTubeVideoListFeed extends AbstractSource implements EventDrivenSource,Configurable, PollableSource{
	  ...
	  private void getVideoList() {
		  ...
		  if(TimeUnit.MILLISECONDS.toSeconds(currentTime - lastPoll) > pollInterval){
			  try {
				  ...
				  bearerClientRequest = new OAuthBearerClientRequest("https://www.googleapis.com/youtube/v3/channels?part=contentDetails&maxResults=50&mine=true&access_token=" + accessToken)
				    .buildQueryMessage();
				    ...
				    bearerClientRequest = new OAuthBearerClientRequest("https://www.googleapis.com/youtube/v3/playlistItems?part=id%2Csnippet%2CcontentDetails%2Cstatus&maxResults=50&playlistId=" + uploads + "&access_token=" + accessToken)
				    .buildQueryMessage();
					bearerClientRequest.addHeader("x-li-format", "json");
					logger.debug(bearerClientRequest.getLocationUri());
						...
						Event e = EventBuilder.withBody(value.getBytes());
						e.getHeaders().put(selectorHeader, "VideoList");
						getChannelProcessor().processEvent(e);
						JSONObject json = (JSONObject) JSONSerializer.toJSON(resourceResponse.getBody().toString());     
						
						while(json.containsKey("nextPageToken"))
						   {
							bearerClientRequest = new OAuthBearerClientRequest("https://www.googleapis.com/youtube/v3/playlistItems?part=id%2Csnippet%2CcontentDetails%2Cstatus&maxResults=50&playlistId=" + uploads + "&access_token=" + accessToken + "&pageToken=" + json.getString("nextPageToken"))
						    .buildQueryMessage();
							bearerClientRequest.addHeader("x-li-format", "json");
							logger.debug(bearerClientRequest.getLocationUri());
