...
return substr($data, $start, $end - $start);
case 'SEARCH':
$start = strpos($data, '<table summary="search">');
$end = strpos($data, '<script', $start);
return substr($data, $start, $end - $start);
break;
}
}
...
$data = $this->stripHead($data, "SEARCH");
$Read = new Read;
$data = explode('<tr><td class="bold l pu">', $data);
$dom = new DOMDocument;
libxml_use_internal_errors(true);
...
$Read->addElement('board', $node->nodeValue);
}
else if($g == 4) {
$Read->addElement('topic', $node->nodeValue);
}
else if($g == 5) {
$Read->addElement('poster', $node->nodeValue);
break;
...
$shares = $node->nodeValue;
$i = 0;
break;}
}
$Read->addElement('likes', (isset($likes) ? $likes : 'No Likes'));
$Read->addElement('shares', (isset($shares) ? $shares : 'No Shares'));
unset($likes);
unset($shares);
}
return $Read;
