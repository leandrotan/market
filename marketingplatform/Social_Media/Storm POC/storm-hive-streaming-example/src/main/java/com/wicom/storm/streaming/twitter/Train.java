...
public class Train {
	...
	void train() throws IOException, ClassNotFoundException {
		...
		classLM= DynamicLMClassifier.createNGramProcess(categories, nGram);
		...
		for (int i = 0; i < categories.length; ++i) {
			...
			for (int j = 0; j < trainFiles.length; ++j) {
				...
				((ObjectHandler) classLM).handle(classified);  
			}
	 	}
		AbstractExternalizable.compileTo((Compilable) classLM, new File("src/classifier.txt"));
	}
}
